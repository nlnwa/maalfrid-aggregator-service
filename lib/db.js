...
async function detectLanguages (detectAll) {
  ...
  detectAndUpdate(cursor, metrics)
    ...
    .catch((error) => r.db('maalfrid').table('system').update({id, endTime: r.now(), metrics, error}).run())
...
async function processAggregate (startTime, endTime) {
  ...
  const cursor = await r.db('maalfrid').table('seeds')('id').run({cursor: true})
  cursor.eachAsync(async (seedId) => {
    const entityId = await r.db('maalfrid').table('seeds').get(seedId)('entityId').run()
    ...
    return query
      ...
      .map(_ => ({
        ...
        seedId,
        executionId: _('group'),
        jobExecutionId: r.db('maalfrid').table('aggregate').getAll(_('group'), {index: 'executionId'}).nth(0)('jobExecutionId'),
        language: _('reduction')
      })).forEach(_ => r.db('maalfrid').table('statistics').insert(_))
      .run()
  })
}
