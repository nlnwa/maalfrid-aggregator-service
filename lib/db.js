 ...
 *
 * @param cursor {Object} cursor to document with text property
 * @returns promise {Promise} promise of metrics object
 */
 ...
 *
 * @param detectAll {boolean} detectAll indicates if already detected texts should be redetected
 * @returns {Promise<*>} promise of id for the entry for this detection run in the system table
 */
 ...
 *
 * @param name {string} only synchronize named entity
 * @param labels {Array} only synchronize entities containing one of labels
 * @returns {Promise<*>} promise of database query result
 */
...
/**
 * Synchronize the seeds of maalfrid entities from corresponding seeds in veidemann
 *
 * @returns {Promise<*>} promise of database query result
 */
...
async function generateAggregate (startTime, endTime) {
  ...
  if (lowerBound >= upperBound) {
    throw error(ERROR.FAILED_PRECONDITION, `lowerbound (${lowerBound}) is greater than or equal to upperbound (${upperBound}`)
  }
...
async function findAggregateLowerBound () {
  ...
  if (!lastAggregation.hasOwnProperty('endTime')) {
    throw error(ERROR.IN_PROGRESS, 'aggregation already in progress')
  } else {
...
async function processAggregate (lowerBound, upperBound, seedId) {
  const previous = await r.db('maalfrid').table('system')
    .filter({ type: 'filter' })
    .orderBy(r.desc('startTime')).run()
  if (previous.length > 0 && !previous[0].hasOwnProperty('endTime')) {
    throw error(ERROR.IN_PROGRESS, 'filtering already in progress')
  }

  const logEntry = { startTime: r.now(), type: 'filter' }
  if (lowerBound && upperBound) {
    Object.assign(logEntry, { lowerBound, upperBound })
  }
  if (seedId) {
    Object.assign(logEntry, { seedId })
  }
  const result = await r.db('maalfrid').table('system').insert(logEntry).run()
  const id = result['generated_keys'][0]

  if (lowerBound && upperBound) {
    await r.db('maalfrid').table('filter').filter(r.row('startTime').during(lowerBound, upperBound)).delete().run()
  }
  ...
  if (globalPredicate) {
    predicates.push(globalPredicate)
  }

  const timePredicate = lowerBound && upperBound
    ? r.row('startTime').during(lowerBound, upperBound)
    : undefined
  if (timePredicate) {
  ...
  const cursor = seedId
    ? await r.db('maalfrid').table('seeds').getAll(seedId).run({ cursor: true })
    : await r.db('maalfrid').table('seeds').run({ cursor: true })

  // deliberatly not await promise returned
  cursor.eachAsync(async ({ id: seedId, entityId }) => {
    let query = r.db('maalfrid').table('aggregate').getAll(seedId, { index: 'seedId' })

    // get the filter for this seed id
    const seedFilter = await r.db('maalfrid').table('filter').getAll(seedId, { index: 'seedId' }).run()
    if (seedFilter && seedFilter.hasOwnProperty('filters')) {
      predicates.push(filterToPredicate(seedFilter['filters']))
    }

    query = predicates.reduce((query, predicate) => query.filter(predicate), query)
    ...
    return query
      ...
      .fold({}, (acc, curr) => {
        ...
        return r.branch(
          acc.hasFields(code),
          acc.merge(r.object(code, {
            total: acc(code)('total').add(1),
            short: acc(code)('short').add(short)
          })),
          acc.merge(r.object(code, {
            total: 1,
            short
    ...
    .then(() => r.db('maalfrid').table('system').update({ id, endTime: r.now() }).run({ noreply: true }))
    .catch(error => r.db('maalfrid').table('system').update({ id, error: error.message }).run({ noreply: true }))
}
