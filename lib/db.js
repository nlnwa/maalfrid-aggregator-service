...
async function detectLanguages (detectAll) {
  ...
  const cursor = detectAll
    ? await query.run({ cursor: true })
    : await filterWithoutLanguages(query).run({ cursor: true })
 ...
 *
 * @param {Cursor} cursor - a cursor to documents containing a 'text' property
 ...
 * @returns {Selection}
 */
function filterMetaByLabels (selection, labels) {
  return labels.reduce((selection, label) => selection.filter((doc) => doc('meta')('label').contains(label)), selection)
}
 ...
 * @returns {Selection}
 */
function filterMetaByName (selection, name) {
  return selection.filter({ meta: { name } })
}
 ...
 * @returns {Selection}
 */
function getSeedsOfEntities () {
  const entityKeys = r.args(r.db(MAALFRID_DB).table(MAALFRID_TABLE_ENTITIES).getField('id').map(id => ['crawlEntity', id]).coerceTo('array'))
  return r.db(VEIDEMANN_DB).table(VEIDEMANN_TABLE_SEEDS).getAll(entityKeys, { index: 'configRefs' })
}
 ...
 * @returns {Selection}
 */
async function aggregateTexts (lowerBound, upperBound) {
...
async function deleteStatistics (lowerBound, upperBound, seedId) {
  return r.db(MAALFRID_DB).table(MAALFRID_TABLE_STATISTICS)
    ...
    .filter(seedId ? { seedId } : {}).delete().run({ returnChanges: false })
}
...
async function processAggregate (lowerBound, upperBound, seedId) {
  ...
  const cursor = seedId
    ? await r.db(MAALFRID_DB).table(MAALFRID_TABLE_SEEDS).getAll(seedId).run({ cursor: true })
    : await r.db(MAALFRID_DB).table(MAALFRID_TABLE_SEEDS).run({ cursor: true })

  return cursor.eachAsync(async ({ id: seedId, entityId }) => {
    /**
     *
     * @type {Selection}
     */
 ...
 * @param {string} seedId
 * @returns {Promise<Statistic[]>} - reQL selection
 */
async function calculateStatistics (selection, entityId, seedId) {
  selection
    .group('executionId')
...
async function setInProgress (type) {
  ...
  if (result.changes.length < 1) {
    ...
    console.log(duration, Date.now(), prev.getTime())
...
async function timeoutOperation (type) {
  // previous operation timed out so update log entries of type without an endTime
...
async function setNotInProgress (type) {
  return r.db(MAALFRID_DB).table(MAALFRID_TABLE_SYSTEM).get('inProgress').update(r.object(type, null)).run({ returnChanges: false })
}
...
async function logEnd (id, meta) {
  return r.db(MAALFRID_DB).table(MAALFRID_TABLE_SYSTEM).update(Object.assign({ id, endTime: r.now() }, meta || {}))
    .run({ returnChanges: false })
}
