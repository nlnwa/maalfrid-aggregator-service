 ...
 * @returns {Promise<string>} id of log entry
 */
async function updateEntities (name, labels, wait = true) {
  ...
  const promise = query.forEach((entity) => r.db(MAALFRID_DB).table(MAALFRID_TABLE_ENTITIES).insert(entity, { conflict: 'replace' })).run()
 ...
 * @returns {Promise<string>} id of log entry
 */
async function updateSeeds (wait = true) {
  const type = TYPE.SYNC_SEEDS

  await setInProgress(type)
  const logId = await logStart(type)

  const promise = r.db(MAALFRID_DB).table(MAALFRID_TABLE_SEEDS)
    .insert(getSeedsOfEntities(), { conflict: 'replace' }).run()
    .then(result => logEnd(logId, { result }))
    .catch(error => logEnd(logId, { error: error.message }))
    .finally(() => setNotInProgress(type))
...
function getSeedsOfEntities () {
  const configRefs = r.db(MAALFRID_DB).table(MAALFRID_TABLE_ENTITIES).getField('id').map(id => ['crawlEntity', id])
  // const entityKeys = r.args(configRefs)
  return r.db(VEIDEMANN_DB).table(VEIDEMANN_TABLE_SEEDS).getAll(configRefs, { index: 'configRefs' })
}
...
function aggregateTexts (lowerBound, upperBound) {
  return r.db(MAALFRID_DB).table(MAALFRID_TABLE_SEEDS).pluck('id')
    ...
    .filter(row => row('right')('state').eq('FINISHED').or(row('right')('state').eq('ABORTED_MANUAL')))
