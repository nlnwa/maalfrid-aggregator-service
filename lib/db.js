...
const { logger, languageDetection, rethinkdb: dbConfig, crawlJob, seed } = require('./config')
const log = logger
...
async function syncSeedsAndEntities (labels, wait = true) {
  ...
  const promise = syncSeeds(labels)
    ...
    .then(entities => logEnd(logId, { result: { seeds, entities } }))
...
async function syncEntities () {
  const entityIds = r.db(MAALFRID_DB).table(MAALFRID_TABLE_SEEDS)('seed')('entityRef')('id').distinct()
...
function filterMetaByLabels (table, labels) {
  ...
  const keyValueForm = labels.map(l => [l.key.toLocaleLowerCase(), l.value.toLocaleLowerCase()])
 ...
 */
async function isJobExecutionId (jobExecutionId) {
  return !!(await r.db(VEIDEMANN_DB).table(VEIDEMANN_TABLE_JOB_EXECUTIONS).get(jobExecutionId).run())
}

async function getCrawlJobId (label = crawlJob.label) {
...
function aggregateTexts (jobExecutionId) {
  ...
  return executions
    ...
    .eqJoin(doc => r.branch(doc.hasFields('warcRefersTo'), doc('warcRefersTo'), doc('warcId')),
...
async function generateAggregate (jobExecutionId, wait = true) {
  ...
  if (jobExecutionId === undefined) {
    jobExecutionId = await getJobExecutionId(await getCrawlJobId())
  } else if (!(await isJobExecutionId(jobExecutionId))) {
...
async function generateStatistics (jobExecutionId, seedId, wait = true) {
  ...
  if (jobExecutionId === undefined || !(await isJobExecutionId(jobExecutionId))) {
    throw new NotFoundError('Could not find job execution with id: ' + jobExecutionId)
  }
