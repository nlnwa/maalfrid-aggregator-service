...
function aggregateTexts (lowerBound, upperBound) {
  const statligeCrawlJobIds = r.db(VEIDEMANN_DB).table(VEIDEMANN_TABLE_CONFIG)
    .getAll(['scope', 'statlige'], { index: 'label' })
    .filter({ kind: 'crawlJob' })
    .getField('id')

  return r.db(VEIDEMANN_DB).table(VEIDEMANN_TABLE_EXECUTIONS)
    .getAll(r.args(statligeCrawlJobIds.coerceTo('array')), { index: 'jobId' })
    // only executions within time range
