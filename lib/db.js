 ...
 *
 * @param {Object} cursor A cursor to documents containing a 'text' property
 * @returns {Promise<Object>} A promise of a metrics object
 */
async function detectAndUpdate (cursor) {
  ...
  async function fn (row) {
    try {
      ...
      metrics[language] = metrics[language] ? metrics[language] + 1 : 1
 ...
 * @returns {Promise<*>} Promise of the id of the log entry in the system table
 */
async function detectLanguages (detectAll) {
  const log = await r.db('maalfrid').table('system').insert({ startTime: r.now(), type: TYPE.LANGUAGE_DETECTION }).run()
 ...
 *
 * @param {string} name Only synchronize named entity
 * @param {Array} labels Only synchronize entities containing one of labels
...
async function findAggregateLowerBound () {
  ...
  if (!lastAggregation.hasOwnProperty('endTime')) {
    throw InProgressError('aggregation already in progress')
  } else {
...
async function processAggregate (lowerBound, upperBound, seedId) {
  ...
  if (previous.length > 0 && !previous[0].hasOwnProperty('endTime')) {
    throw InProgressError('filtering already in progress')
  }

  const logEntry = { startTime: r.now(), type: TYPE.FILTER }
  if (lowerBound && upperBound) {
  ...
  // deliberately not await promise returned
