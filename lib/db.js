...
async function detectLanguages (request) {
  const {detectAll} = request
...
function updateEntities (request) {
  ...
  if (name) {
    query = query.filter(doc => doc('meta')('name').eq(name))
  }
  return query.forEach((entity) => r.db('maalfrid').table('entities').insert(entity, {conflict: 'replace'})).run()
}
 ...
 * @returns {Promise}
 */
async function generateAggregate (request) {
